...
date is set equal the loan date.
...
these products is requested by including its own child object (the only products
allowed for single payment notes are `Life` and `Disability`), along with the
borrower birthdays.
